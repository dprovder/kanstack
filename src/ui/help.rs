...
pub(super) fn draw_help(f: &mut Frame, area: Rect, hits: &mut HitMap) {
    let body = vec![
        ...
        help_row("f", "resolve conflicts on this lane, file by file"),
        help_row("  then o / t", "take ours / theirs for the selected file"),
        help_row("  then A", "hand it to this lane's coding agent as a task"),
        ...
        Line::raw(""),
        help_row("current lane", "tinted header — the one ←/→ and m/c/p/L/d act on"),
