//! End-to-end checks of `kanstack claim` — the `PreToolUse` hook contract (see
...
//! `kanstack claim` invocations, standing in for two lanes' `PreToolUse` hooks firing against
//! the same file, driven by real `kanstack report` calls in between rather than a live harness.
